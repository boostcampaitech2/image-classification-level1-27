...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label['age'] = torch.tensor(self.ages[idx], dtype=torch.long)
...
class CustomDataset_18(Dataset):
    def __init__(self, args, train=True):
        super().__init__()
        self.data_dir = args.data_dir
        self.info_path = args.info_path
...
class CustomDataset_18_agethresh(Dataset):
    def __init__(self, args, train=True):
        super().__init__()
        self.data_dir = args.data_dir
        self.info_path = args.info_path
...
class CustomDataset_18_Aug60(Dataset):
    def __init__(self, args, train=True):
        super().__init__()
        self.data_dir = args.data_dir
        self.aug_dir = '/opt/ml/input/data/train/augmentation_crop_images'
        self.info_path = args.info_path
        self.k = args.n_splits
        self.seed = args.seed
        self.k_index= args.k_index
        ...
        for directory in tqdm(self.folders):
            image_dir = os.path.join(self.data_dir, directory)
            ID, GENDER, RACE, real_AGE = directory.split('_')
            _ID, _GENDER, _RACE, _AGE = directory.split('_')
            ...
            if self.train and (AGE==2):
                f = '_'.join([_ID+'-1', _GENDER, _RACE, _AGE])
                aug_folders.append(f)

        if self.train:
            for directory in aug_folders:
                image_dir = os.path.join(self.aug_dir, directory)
                _ID, _GENDER, _RACE, _AGE = directory.split('_')
